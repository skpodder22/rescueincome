    ...
    if (state == true){
       // check for loaded class availability
        if (this.pageOverlayElement.className.match(/\bloaded\b/)){
           ...
           */
           console.log({state:state,data:this.pageOverlayElement.className.replace(/\bloaded\b/, "loading")});
...
// event trigger when everything is loaded
 function onloadFalse(){
  console.log('false');
    pixelvarsPageLoading.loading(false);
}

window.onload = onloadFalse;
window.onresize = onloadFalse;
