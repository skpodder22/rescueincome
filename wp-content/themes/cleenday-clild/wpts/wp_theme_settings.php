...
class wp_theme_settings{
	...
	private function binput($array){ 
		...
		switch ($array['type']) {
			...
			case 'select':
				...
				if(empty($arr)){
					if(!empty($array['content_more_wraper'])){

						echo $array['content_more_wraper'];
					}
					$sle = '<select name="'.$array['name'].'" class="'.$html_class.'">';
					
						foreach ($array['options'] as $key => $value) {
							$sle .= '<option value="'.$this->keyEntity($key).'" '.($this->wpts_option($array['name']) == $key ? 'selected' : '').'>'.$value.'</option>';
						}
					$sle .= '</select>';
						echo $sle;

					if(!empty($array['content_more_wraper_end'])){
						echo $array['content_more_wraper_end'];
					}
				}else{
					foreach($arr as $val){
						...
						foreach ($array['options'] as $key => $value) {
							$sle .= '<option value="'.$this->keyEntity($key).'" '.($val == $key ? 'selected' : '').'>'.$value.'</option>';
						}
					$sle .= '</select>';
				...
				if(!empty($array['add_more_button'])){
					echo $array['add_more_button'];
				}
				?>
	...
	private function navTabs(){
		...
		echo '<form method="post" class="nav-rtab-form" action="">';
	...
	public function wpts_update_admin_notice() {
		...
		//$this->get_wpts_git_version()
		$message = __( 'New version of WPTS is available (1), click <a href="https://git.io/vi1Gr" target="_new">here</a> to learn more about it.');
