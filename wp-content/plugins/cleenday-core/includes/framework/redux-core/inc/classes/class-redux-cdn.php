				...
				if ( is_wp_error( $cdn_response ) || 200 !== wp_remote_retrieve_response_code( $cdn_response ) ) {
					if ( class_exists( 'Redux_VendorURL' ) ) {
						...
						if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
							$msg = esc_html__( 'Please wait a few minutes, then try refreshing the page. Unable to load some remotely hosted scripts.', 'cleenday-core' );
							if ( self::$parent->args['dev_mode'] ) {
								// translators: %s: URL.
								$msg = sprintf( esc_html__( 'If you are developing offline, please download and install the %s plugin/extension to bypass our CDN and avoid this warning', 'cleenday-core' ), '<a href="https://github.com/reduxframework/redux-vendor-support" target="_blank">Redux Vendor Support</a>' );
							}

							// translators: %s: CDN handle.
							$msg = '<strong>' . esc_html__( 'Redux Framework Warning', 'cleenday-core' ) . '</strong><br/>' . sprintf( esc_html__( '%s CDN unavailable.  Some controls may not render properly.', 'cleenday-core' ), $handle ) . '  ' . $msg;
		...
		private static function vendor_plugin( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
			if ( class_exists( 'Redux_VendorURL' ) ) {
				...
				if ( ! self::$set ) {
					...
					$msg = sprintf( esc_html__( 'The %1$s (or extension) is either not installed or not activated and thus, some controls may not render properly.  Please ensure that it is installed and %2$s', 'cleenday-core' ), '<a href="https://github.com/reduxframework/redux-vendor-support">Vendor Support plugin</a>', '<a href="' . admin_url( 'plugins.php' ) . '">' . esc_html__( 'activated.', 'cleenday-core' ) . '</a>' );
