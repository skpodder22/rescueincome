	...
	class Redux_Connection_Banner {
		...
		public function admin_ajax() {
			...
			if ( empty( $nonce ) || ! wp_verify_nonce( $nonce, $this->nonce ) ) {
				die( __( 'Security check failed.', 'cleenday-core' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		public static function enqueue_connect_button_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'buttonTextRegistering' => __( 'Loading...', 'cleenday-core' ),
		...
		public function panel_admin_notice( $args ) {
			...
			if ( empty( $this->notice ) ) {
				...
				// }
				$notice = sprintf( __( 'Register <strong>%1$s</strong> to enable automatic Google Font updates service. Plus unlock all free block templates in the Redux template library.', 'cleenday-core' ), $name );
			} else {
				$notice = $this->notice;
			}

			$notice .= ' (<a class="redux-insights-data-we-collect" href="#" style="white-space: nowrap;">' . __( 'learn more', 'cleenday-core' ) . '</a>)';
			...
			echo '&nbsp;<a href="' . esc_url( $urls['register'] ) . '" class="button-primary button-large redux-activate-connection redux-connection-banner-action" data-url="' . admin_url( 'admin-ajax.php' ) . '" data-activate="panel_banner">' . __( 'Register Now', 'cleenday-core' ) . '</a>';
			echo '&nbsp;&nbsp;&nbsp;<a href="' . esc_url( $urls['dismiss'] ) . '" style="color: #aaa;" class="redux-connection-banner-action" data-activate="false" data-url="' . admin_url( 'admin-ajax.php' ) . '">' . __( 'No thanks', 'cleenday-core' ) . '</a>';
			echo '</p></div>';
		...
		public function render_banner() {
			...
			<div id="redux-connect-message" class="updated redux-banner-container" data-nonce="<?php echo wp_create_nonce( $this->nonce ); ?>">
				<div class="redux-banner-container-top-text">
					...
					<span>
						<?php esc_html_e( 'You’re almost done. Register for our service to unlock even more tools to help you build better sites faster in WordPress.', 'cleenday-core' ); ?>
					</span>
				</div>
				<div class="redux-banner-inner-container">
					<a href="<?php echo esc_url( $urls['dismiss'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>"
					   ...
					   title="<?php esc_attr_e( 'Dismiss this notice', 'cleenday-core' ); ?>" data-activate="false">
					</a>
					...
					<div class="redux-banner-content-container">
						...
						<div class="redux-banner-slide redux-banner-slide-one redux-slide-is-active">
							...
							<div class="redux-banner-slide-text">
								<h2><?php esc_html_e( 'Build better sites faster with Redux', 'cleenday-core' ); ?></h2>
								<p>
									<?php
									esc_html_e(
								...
								<div class="redux-banner-button-container">
									...
									<a href="<?php echo esc_url( $urls['dismiss'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>"
									   ...
									   title="<?php esc_attr_e( 'No thanks', 'cleenday-core' ); ?>" data-activate="false"><?php esc_html_e( 'No Thanks', 'cleenday-core' ); ?></a>&nbsp;&nbsp;
									...
									<a href="<?php echo esc_url( $urls['register'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-activate="main_banner"
									   class="button button-primary button-large redux-alt-connect-button redux-connection-banner-action">
										<?php esc_html_e( 'Register', 'cleenday-core' ); ?>
									</a>
								</div>
				...
				<div class="redux-banner-full-container">
					<div class="redux-banner-full-container-card">
						...
						<div class="redux-banner-full-step-header">
							<h2 class="redux-banner-full-step-header-title"><?php esc_html_e( 'Activate essential WordPress security and performance tools by setting up Redux', 'cleenday-core' ); ?></h2>
						</div>
							...
							<div class="redux-banner-full-slide">
								<div class="redux-banner-full-slide-card illustration">
									<img
										...
										alt="<?php esc_attr_e( 'Security & Backups', 'cleenday-core' ); ?>"
									/>
								</div>
							...
							<div class="redux-banner-full-slide">
								<div class="redux-banner-full-slide-card illustration">
									<img
										...
										alt="<?php esc_attr_e( 'Built-in Performance', 'cleenday-core' ); ?>"
									/>
								</div>
								<div class="redux-banner-full-slide-card">
									<p>
		...
		public static function tos_blurb( $campaign = 'options_panel' ) {
			return sprintf(
				__( 'By clicking the <strong>Register</strong> button, you agree to our <a href="%1$s" target="_blank">terms of service</a>, to create an account, and to share details of your usage metrics with Redux.io.', 'cleenday-core' ),
