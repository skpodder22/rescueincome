		...
		public function __construct( $parent, $args ) {
			...
			$default = array(
				...
				'font_weights'                     => array(
					array(
						'id'   => '400',
						'name' => __( 'Regular 400', 'cleenday-core' ),
					),
					array(
						'id'   => '400italic',
						'name' => __( 'Regular 400 Italic', 'cleenday-core' ),
					),
					array(
						'id'   => '700',
						'name' => __( 'Bold 700', 'cleenday-core' ),
					),
					array(
						'id'   => '700italic',
						'name' => __( 'Bold 700 Italic', 'cleenday-core' ),
					),
				),
		...
		private function no_errors_please( $args ) {
			...
			if ( empty( $args['footer_credit'] ) ) {
				...
				/* translators: 1: Redux, 2: Link to plugin review */
					__( 'Enjoyed %1$s? Please leave us a %2$s rating. We really appreciate your support!', 'cleenday-core' ),
					'<strong>' . __( 'Redux', 'cleenday-core' ) . '</strong>',
			...
			if ( empty( $args['menu_title'] ) ) {
				$args['menu_title'] = esc_html__( 'Options', 'cleenday-core' );
			}

			if ( empty( $args['page_title'] ) ) {
				$args['page_title'] = esc_html__( 'Options', 'cleenday-core' );
			}
