...
class Redux_User_Feedback {
	...
	public function seconds_to_words( $seconds ) {
		...
		if ( $years > 1 ) {
			...
			return sprintf( __( '%s years', 'cleenday-core' ), $years );
		} elseif ( $years > 0 ) {
			return __( 'a year', 'cleenday-core' );
		}
		...
		if ( $weeks > 1 ) {
			...
			return sprintf( __( '%s weeks', 'cleenday-core' ), $weeks );
		} elseif ( $weeks > 0 ) {
			return __( 'a week', 'cleenday-core' );
		}
		...
		if ( $days > 1 ) {
			...
			return sprintf( __( '%s days', 'cleenday-core' ), $days );
		} elseif ( $days > 0 ) {
			return __( 'a day', 'cleenday-core' );
		}
		...
		if ( $hours > 1 ) {
			...
			return sprintf( __( '%s hours', 'cleenday-core' ), $hours );
		} elseif ( $hours > 0 ) {
			return __( 'an hour', 'cleenday-core' );
		}
		...
		if ( $minutes > 1 ) {
			...
			return sprintf( __( '%s minutes', 'cleenday-core' ), $minutes );
		} elseif ( $minutes > 0 ) {
			return __( 'a minute', 'cleenday-core' );
		}
		...
		if ( $seconds > 1 ) {
			...
			return sprintf( __( '%s seconds', 'cleenday-core' ), $seconds );
		} elseif ( $seconds > 0 ) {
			return __( 'a second', 'cleenday-core' );
		}
	}
			...
			<div class="notice updated redux-notice">
				<div class="redux-notice-inner">
					<div class="redux-notice-icon">
						...
						<img src="<?php echo esc_url( Redux_Core::$url . '/assets/img/icon--color.svg' ); ?>" alt="<?php printf( esc_attr__( '%s WordPress Plugin', 'cleenday-core' ), esc_attr( $this->name ) ); ?>" />
					</div>
					<div class="redux-notice-content">
						<?php /* translators: 1. Name */ ?>
						<h3><?php printf( esc_html__( 'Are you enjoying %s?', 'cleenday-core' ), esc_html( $this->name ) ); ?></h3>
						<p>
							<?php /* translators: 1. Name, 2. Time */ ?>
							<?php printf( esc_html__( 'You have been using %1$s for %2$s now. Would you mind leaving a review to let us know know what you think? We\'d really appreciate it!', 'cleenday-core' ), esc_html( $this->name ), esc_html( $time ) ); ?>
						</p>
					</div>
					<div class="redux-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary redux-install-button" target="_blank">%2$s</a>', esc_url( 'https://wordpress.org/support/plugin/redux-framework/reviews/?filter=5#new-post' ), esc_html__( 'Leave a Review', 'cleenday-core' ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', 'cleenday-core' ); ?></a>
					</div>
				</div>
			</div>
			<?php
