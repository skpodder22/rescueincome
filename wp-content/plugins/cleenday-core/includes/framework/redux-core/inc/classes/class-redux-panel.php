...
if ( ! class_exists( 'Redux_Panel', false ) ) {
	...
	class Redux_Panel {
		...
		private function panel_template() {
			...
			// Do we support JS?
			echo '<noscript><div class="no-js">' . esc_html__( 'Warning- This options panel will not work properly without javascript!', 'cleenday-core' ) . '</div></noscript>';
		...
		public function notification_bar() {
			if ( isset( $this->parent->transients['last_save_mode'] ) ) {
				...
				if ( 'import' === $this->parent->transients['last_save_mode'] ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'cleenday-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", esc_html__( 'All Defaults Restored!', 'cleenday-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", esc_html__( 'Section Defaults Restored!', 'cleenday-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Saved!', 'cleenday-core' ) ) ) . '</strong>';
					echo '</div>';
				}
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			echo '<strong>' . esc_html( apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'cleenday-core' ) ) ) . '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'error(s) were found!', 'cleenday-core' );
			echo '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'warning(s) were found!', 'cleenday-core' );
			echo '</strong>';
			echo '</div>';
		}
				...
				if ( $developer_theme_file ) {
					...
					if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) && isset( $this->parent->args['dev_mode'] ) && ! empty( $this->parent->args['dev_mode'] ) ) {
						...
						<div id="message" class="error redux-message">
							<p>
								<strong><?php esc_html_e( 'Your panel has bundled copies of Redux Framework template files that are outdated!', 'cleenday-core' ); ?></strong>&nbsp;&nbsp;<?php esc_html_e( 'Please update them now as functionality issues could arise.', 'cleenday-core' ); ?></a></strong>
							</p>
						</div>
						<?php
