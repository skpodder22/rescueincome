				...
				foreach ( $files as $file ) {
					...
					if ( $theme_file ) {
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( '<code>%s</code> ' . esc_html__( 'version', 'cleenday-core' ) . ' <strong style="color:red">%s</strong> ' . esc_html__( 'is out of date. The core version is', 'cleenday-core' ) . ' %s', str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
