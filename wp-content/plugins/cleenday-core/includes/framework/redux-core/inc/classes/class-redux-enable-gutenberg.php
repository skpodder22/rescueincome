	...
	class Redux_Enable_Gutenberg {
		...
		public function display_admin_notice() {
			...
			$data = array(
				'url'     => '',
				'content' => '',
				'header'  => __( 'Gutenberg is currently disabled!', 'cleenday-core' ),
				'button'  => '',
			);
			...
			if ( isset( $_GET[ $this->decativate_option ] ) || empty( self::$known_plugins ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET[ $this->decativate_option ] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					// That didn't work.
					$data['header']  = __( 'Hmm, it seems something else is disabling Gutenberg...', 'cleenday-core' );
					...
					$data['header']   = __( 'Your theme author has disabled Gutenberg!', 'cleenday-core' );
					...
					$data['header']   = __( 'Looks like something has disabled Gutenberg?', 'cleenday-core' );
				...
				$data['button'] = __( 'Enable Gutenberg', 'cleenday-core' );

			} elseif ( empty( self::$known_plugins ) ) {
				// Disabled by the theme or other.
				$data['header'] = __( 'Your theme', 'cleenday-core' );
				$data['url']    = $auto_enable_url;
				$data['button'] = __( 'Enable Gutenberg', 'cleenday-core' );
			} else {
				// Disable Plugins!
				...
				if ( 1 === count( self::$known_plugins ) ) {
					$data['button']  = __( 'Disable Plugin', 'cleenday-core' );
					...
					$data['button']  = __( 'Disable Plugins', 'cleenday-core' );
			...
			<div class="notice updated redux-notice">
				<div class="redux-notice-inner">
					<div class="redux-notice-icon">
						...
						<img src="<?php echo esc_url( Redux_Core::$url . '/assets/img/icon--color.svg' ); ?>" alt="<?php echo esc_attr__( 'Redux WordPress Plugin', 'cleenday-core' ); ?>"/>
					</div>
					...
					<div class="redux-install-now">
						...
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No Thanks', 'cleenday-core' ); ?></a>
					</div>
				</div>
			</div>
			<?php
