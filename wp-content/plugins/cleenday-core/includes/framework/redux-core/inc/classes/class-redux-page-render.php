	...
	class Redux_Page_Render extends Redux_Class {
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'cleenday-core' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'cleenday-core' );
				}
				...
				$event = esc_html__( 'moving the mouse over', 'cleenday-core' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'cleenday-core' );
				}

				// Construct message.
				// translators: %1$s: Mousr action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'cleenday-core' ), $event, Redux_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'redux-hint-tab',
					'title'   => esc_html__( 'Hints', 'cleenday-core' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				// about hints.
				if ( true === $this->show_hints ) {
					$screen->set_help_sidebar( '<p><strong>Redux Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'cleenday-core' ) . '</p>' );
				}
			}
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							echo sprintf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'cleenday-core' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
		...
		private function get_default_output_string( $field ) {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'cleenday-core' ) . ': ' . substr( $default_output, 0, - 2 );
			}
