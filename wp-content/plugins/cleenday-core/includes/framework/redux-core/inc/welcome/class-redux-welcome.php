...
if ( ! class_exists( 'Redux_Welcome', false ) ) {
	...
	class Redux_Welcome {
		...
		public function do_redirect() {
			if ( ! defined( 'WP_CLI' ) ) {
				wp_safe_redirect( esc_url( admin_url( add_query_arg( array( 'page' => 'cleenday-core' ), 'tools.php' ) ) ) );
				exit();
			}
		}
		...
		public function change_wp_footer() {
			echo esc_html__( 'If you like', 'cleenday-core' ) . ' <strong>Redux</strong> ' . esc_html__( 'please leave us a', 'cleenday-core' ) . ' <a href="https://wordpress.org/support/view/plugin-reviews/redux-framework?filter=5#postform" target="_blank" class="redux-rating-link" data-rated="Thanks :)">&#9733;&#9733;&#9733;&#9733;&#9733;</a> ' . esc_html__( 'rating. A huge thank you in advance!', 'cleenday-core' );
		}
		...
		public function admin_menus() {
			...
			// About Page.
			$page( esc_html__( 'What is Redux Framework?', 'cleenday-core' ), esc_html__( 'Redux Framework', 'cleenday-core' ), $this->minimum_capability, 'redux-framework', array( $this, 'about_screen' ) );

			// Support Page.
			$page( esc_html__( 'Get Support', 'cleenday-core' ), esc_html__( 'Get Support', 'cleenday-core' ), $this->minimum_capability, 'redux-support', array( $this, 'get_support' ) );

			// Status Page.
			$page( esc_html__( 'Redux Health Check', 'cleenday-core' ), esc_html__( 'Redux Health Check', 'cleenday-core' ), $this->minimum_capability, 'redux-health', array( $this, 'heath_check' ) );
		...
		public function tabs() {
			...
			<h2 class="nav-tab-wrapper">
				<a
					...
					href="<?php echo esc_url( admin_url( add_query_arg( array( 'page' => 'cleenday-core' ), 'tools.php' ) ) ); ?>">
					<?php esc_attr_e( 'What is Redux?', 'cleenday-core' ); ?>
				</a>
				<a
					...
					<?php esc_attr_e( 'Health Check', 'cleenday-core' ); ?>
				</a>
				<a
					...
					<?php esc_attr_e( 'Get Support', 'cleenday-core' ); ?>
				</a>
			...
			<p class="redux-actions">
				...
				if ( $options['demo'] ) {
					...
					<a
						...
						class=" button-text button-demo"><?php echo esc_html__( 'Disable Panel Demo', 'cleenday-core' ); ?></a>
					<?php
				} else {
					?>
					<a
						href="<?php echo esc_url( admin_url( add_query_arg( $query_args, 'tools.php' ) ) ); ?>"
						class=" button-text button-demo active"><?php echo esc_html__( 'Enable Panel Demo', 'cleenday-core' ); ?></a>
					<?php
				}
