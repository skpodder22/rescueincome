...
if ( ! class_exists( 'Redux_Text', false ) ) {
	...
	class Redux_Text extends Redux_Field {
		...
		public static function make_descriptor() {
			...
			$d->set_info( 'Text', __( 'The Text field accepts any form of text and optionally validates the text before saving the value.', 'cleenday-core' ) );

			$d->add_field( 'text_hint', __( 'Display a qTip div when active on the text field.', 'cleenday-core' ), RDT::TEXT )->set_order( 100 )->set_required();
			$d->add_field( 'placeholder', __( 'Placeholder text.', 'cleenday-core' ), RDT::TEXT )->set_order( 100 )->set_required();
		}
