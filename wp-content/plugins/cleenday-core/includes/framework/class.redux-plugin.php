...
if ( ! class_exists( 'Redux_Framework_Plugin', false ) ) {
	...
	class Redux_Framework_Plugin {
		...
		public static function instance() {
			...
			if ( function_exists( 'get_plugin_data' ) && file_exists( $path ) ) {
				...
				if ( is_plugin_active( 'redux-framework/redux-framework.php' ) && true === $res ) {
					echo '<div class="error"><p>' . esc_html__( 'Redux Framework version 4 is activated but not loaded. Redux Framework version 3 is still installed and activated.  Please deactivate Redux Framework version 3.', 'cleenday-core' ) . '</p></div>'; // phpcs:ignore WordPress.Security.EscapeOutput
					return;
				}
			}
		...
		public static function deactivate( $network_wide ) {
			...
			Redux_Enable_Gutenberg::cleanup_options( 'cleenday-core' ); // Auto disable Gutenberg and all that.
		}
		...
		private static function single_activate() {
			...
			$notices[] = esc_html__( 'Redux Framework has an embedded demo.', 'cleenday-core' ) . ' <a href="./plugins.php?redux-framework-plugin=demo&nonce=' . $nonce . '">' . esc_html__( 'Click here to activate the sample config file.', 'cleenday-core' ) . '</a>';
		...
		public function options_toggle_check() {
			...
			if ( isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'redux_framework_demo' ) ) {
				if ( isset( $_GET['redux-framework-plugin'] ) && 'demo' === $_GET['redux-framework-plugin'] ) {
					$url = admin_url( add_query_arg( array( 'page' => 'cleenday-core' ), 'tools.php' ) );
		...
		public function plugin_metalinks( $links, $file ) {
			if ( strpos( $file, 'redux-framework.php' ) !== false && is_plugin_active( $file ) ) {
				$links[] = '<a href="' . esc_url( admin_url( add_query_arg( array( 'page' => 'cleenday-core' ), 'tools.php' ) ) ) . '">' . esc_html__( 'What is this?', 'cleenday-core' ) . '</a>';
				$links[] = '<a href="' . esc_url( admin_url( add_query_arg( array( 'post_type' => 'page' ), 'post-new.php' ) ) ) . '#redux_templates=1">' . esc_html__( 'Template Library', 'cleenday-core' ) . '</a>';
			}
