...
function wpmlm_get_user_tax_details_by_username($username) {
    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $sql = "SELECT * FROM {$table_prefix}tax_details WHERE username = '" . $username . "'";
    $results = $wpdb->get_row($sql);
    return $results;
}
...
function wpmlm_insert_leg_amount($user_id, $package_id) {   
   ...
   global $wpdb;
    ...
    foreach ($result as $res) {
        ...
        $test = $wpdb->insert($table_name, $data);
        //print_r($test);die('data');
