...
function create_wpmlm_users_table() {
    ...
    PRIMARY KEY (`user_id`) ) $charset_collate AUTO_INCREMENT=1";
...
function wpmlm_delete_user_data() {
    ...
    $table_prefix=$wpdb->prefix;
    $table_name = $wpdb->prefix . "wpmlm_users";
    $usr_count = "SELECT COUNT(user_ref_id) as count FROM {$table_name}";
    $count_result = $wpdb->get_results($usr_count);
    foreach($count_result as $number){
        $count_total = $number->count; 
    }
    if($count_total > 1){
...
function create_wpmlm_country_table() {
    ...
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}wpmlm_country
                        (
...
function create_wpmlm_registration_packages_table() {
    ...
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}wpmlm_registration_packages (
...
function create_wpmlm_level_table() {
    ...
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}wpmlm_level_commission (
...
function create_wpmlm_tax_details_table() {

    global $wpdb;
    $table_prefix = $wpdb->prefix;
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS {$table_prefix}tax_details (
    `id` int(20) NOT NULL AUTO_INCREMENT,
    `username` varchar(20) NOT NULL DEFAULT '0',
    `tax_password` varchar(250) NOT NULL,
    `agency` varchar(250) NOT NULL DEFAULT '0',
    PRIMARY KEY (`id`)
    ) {$charset_collate} AUTO_INCREMENT=1";
...
function insert_wpmlm_country_data() {
    ...
    $charset_collate = $wpdb->get_charset_collate();
    $rowcount = "SELECT COUNT(*) FROM {$table_name}";
    $num = $wpdb->get_var($rowcount);
    if($num<1){
    $sql = "INSERT INTO $table_name (`id`, `iso`, `name`, `iso3`, `numcode`) VALUES
